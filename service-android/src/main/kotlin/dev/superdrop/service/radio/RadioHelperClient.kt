 ...
 *
 * WHAT THIS IS
 *   Super Drop's copy of the canonical universal-radio-helper client
 ...
 *   (pkg `dev.superdrop.radiohelper`) instead of re-implementing the
 *   targetSdk-28 legacy toggle / self-ADB / Shizuku ladder in this app.
 *
 * WHO CALLS IT (in Super Drop)
 *   [dev.superdrop.service.receiver.ReceiverForegroundService] on an NFC
 ...
 *   - Manifest: `<uses-permission android:name="dev.superdrop.radiohelper.permission.BIND_RADIO"/>`
 *     + `<queries><package android:name="dev.superdrop.radiohelper"/></queries>`.
 *   - Super Drop must be signed with the SAME key as the helper (BIND_RADIO is a
