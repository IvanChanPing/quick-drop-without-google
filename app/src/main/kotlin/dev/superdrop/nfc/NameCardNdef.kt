 ...
 * docs/NAMECARD_V2_EXECUTOR_PLAN.md §5–§6, Appendix A2).
 *
 ...
 *
 * MESSAGE = two records:
 *  - Record 1 ([NdefRecord.createExternal]): external type `superdrop.dev:namecard`,
 ...
 *
 * WHO CALLS:
 *  - CARD side [SuperDropNdefApduService] serves `build(token, pkg).toByteArray()` as its Type-4 NDEF
 *    file (it needs the raw bytes for `buildNdefFile`).
 *  - READER side [dev.superdrop.namecard.NameCardTransferActivity] passes the OS-delivered
 *    `NdefMessage` straight to [parseToken].
 *
 * Domain+type are ALL-LOWERCASE, matching the manifest `pathPrefix="/superdrop.dev:namecard"` so the
...
internal object NameCardNdef {
    ...
    const val EXT_DOMAIN: String = "superdrop.dev"
    ...
    /** Exact type bytes (`superdrop.dev:namecard`) an incoming record must carry — see [parseToken]. */
     ...
     * `superdrop.dev:namecard` record. MUST match our exact external type: the AAR is ALSO a
