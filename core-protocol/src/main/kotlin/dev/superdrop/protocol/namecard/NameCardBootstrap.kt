 ...
 * advertises/scans for it, so a third nearby Super Drop phone can't hijack the
 * pairing.
 *
 * Wire format (fixed [SIZE] = 17 bytes): `version(1) | token(16)`.
 *
 * Status: pure-JVM, unit-tested in `NameCardBootstrapTest`. The NFC HCE/reader
 * that emit/parse it and the BLE rendezvous that consumes it are device-verified.
 */
