...
import dev.superdrop.protocol.crypto.D2DKeyDerivation
import dev.superdrop.protocol.crypto.D2DRole
import dev.superdrop.protocol.crypto.pin.PinDerivation
import dev.superdrop.protocol.crypto.securemessage.SecureChannel
import dev.superdrop.protocol.medium.Medium
import dev.superdrop.protocol.medium.MediumRegistry
import dev.superdrop.protocol.payload.PayloadAssembler
import dev.superdrop.protocol.payload.PayloadEvent
import dev.superdrop.protocol.payload.PayloadTransferEncoder
import dev.superdrop.protocol.qr.QrHandshakeSigner
import dev.superdrop.protocol.sharing.OutboundSharingFsm
import dev.superdrop.protocol.sharing.OutboundSharingState
import dev.superdrop.protocol.sharing.PairedKeyEncryptionFrame
import dev.superdrop.protocol.sharing.SharingFrame
import dev.superdrop.protocol.sharing.SharingFrameType
import dev.superdrop.protocol.sharing.SharingFrameVersion
import dev.superdrop.protocol.sharing.SharingFrames
import dev.superdrop.protocol.sharing.SharingFsmEffect
import dev.superdrop.protocol.sharing.SharingFsmEvent
import dev.superdrop.protocol.sharing.SharingV1Frame
import dev.superdrop.protocol.transport.ConnectedTransport
import dev.superdrop.protocol.transport.EndOfFrameStream
import dev.superdrop.protocol.transport.FramedConnection
import dev.superdrop.protocol.ukey2.Ukey2Client
import dev.superdrop.protocol.ukey2.Ukey2HandshakeResult
...
internal class OutboundConnectionDriver(
     ...
     *   stock Android Quick Share, and Bada itself, which advertises version 1).
     *
    ...
    private suspend fun streamFilesAndComplete(channel: SecureChannel): OutboundResult {
        ...
        // 7+, stock Android Quick Share, Bada). A version-0 peer — notably Windows Quick
        // Share — treats a Disconnection that arrives BEFORE it reaches kComplete as a
        // FAILURE ("Can't complete transfer") even though every byte already arrived.
        // For such a peer we do NOT disconnect here; we let it finalize and close first.
        // runReceiveLoop's safe-disconnect drain (shouldDrainForSafeDisconnect is true
        // for Completed) then waits for the peer's FIN/Disconnection or a grace timeout
        // before we close — mirroring stock Quick Share, whose successful transfers end
        // as a receiver-driven LOCAL_DISCONNECTION after kComplete.
        if (peerSafeToDisconnectVersion >= 1) {
            ...
            logger(
                "fsm: all files streamed; peer safeToDisconnectVersion=" +
                    "$peerSafeToDisconnectVersion (safe-to-disconnect disabled, e.g. Windows) " +
                    "— NOT sending eager Disconnection, waiting for receiver to finalize + close (#200)",
            )
        }
